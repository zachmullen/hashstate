...
static PyObject *
EVP_serialize(EVPobject *self, PyObject* unused)
{
  ...
  for(i=0; i<ctx_size; i++) {
      serialized[i] = ((unsigned char*)self->ctx.md_data)[i];
  }
...
PyDoc_STRVAR(EVP_deserialize__doc__,
"Construct a hash object from a serialized version of the state.");

static PyObject *
EVP_deserialize(EVPobject *self, PyObject* args)
{
  char* state;
  int len;
  int i;
  ...
  if (self->lock != NULL) {
      ...
      for(i=0; i<len; i++) {
        ((char*)self->ctx.md_data)[i] = state[i];
      }
...
static PyMethodDef EVP_methods[] = {
    ...
    {"serialize",   (PyCFunction)EVP_serialize,   METH_NOARGS,  EVP_serialize__doc__},
    {"deserialize", (PyCFunction)EVP_deserialize, METH_VARARGS, EVP_deserialize__doc__},
    {NULL, NULL}  /* sentinel */
};
...
#define CONSTRUCTOR_METH_DEF(NAME)  \
    { #NAME, (PyCFunction)EVP_new_ ## NAME, METH_VARARGS, \
...
PyInit_hashstate(void)
{
    ...
    m = PyModule_Create(&hashstatemodule);
    if (m == NULL)
        return NULL;
